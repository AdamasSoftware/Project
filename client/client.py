...
def main():
    ...
    try:
        ...
        if ((parsed_url.port == None) or (parsed_url.path == '') or (parsed_url.path == '/') or (
    ...
    if response_list[1] == '301':
        ...
        while (not headers_done):
            ...
            if (header_line == ''):
                ...
                # store new location of server sent in header
            elif (header_list[0] == 'Location:'):
                server_info = header_list[1]
        server_info = "http://" + server_info
        ...
        try:
            parsed_url = urlparse(server_info)
            if ((parsed_url.port == None) or (parsed_url.path == '') or (parsed_url.path == '/') or (
                    parsed_url.hostname == None)):
                raise ValueError
            host = parsed_url.hostname
